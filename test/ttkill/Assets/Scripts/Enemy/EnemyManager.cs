...
public class EnemyManager : MonoBehaviour {
	...
	{
		instance = this;
		if (SettingManager.Instance.TutorialSeq == 30)
		{
			TutorialStoryDialog.Popup(Init);
		}
		else
		{
			Init();
	...
	IEnumerator FireTrigger(float index)
	{
		...
		if (SettingManager.Instance.TutorialSeq == index)
		{
			...
			else if (SettingManager.Instance.TutorialSeq == index)
			{
				StartCoroutine(FireTrigger(index));
			}
		}
	}
