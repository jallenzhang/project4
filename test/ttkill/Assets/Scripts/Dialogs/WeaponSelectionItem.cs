	...
	public WeaponType WeaponType
	{
		...
		set
		{
			...
			if (level > 0)
			{
				...
				tt.needAuto = true;
				tt.direction = TutorialDir.Down;
				tt.m_needDescription = true;
				tt.m_description = "装配满武器才可以进行战斗";
				if (weaponType == WeaponType.dao && SettingManager.Instance.TutorialSeq <= 14)
				{
					tt.index = 14;
					StartCoroutine(FireTrigger());
				}
				else if (weaponType == WeaponType.gun_shouqiang && SettingManager.Instance.TutorialSeq <= 14)
				{
					tt.index = 15;
				}
				else if (weaponType == WeaponType.gun_sandan && SettingManager.Instance.TutorialSeq <= 14)
				{
					tt.index = 16;
				}
				else if (weaponType == WeaponType.gun_liudan && SettingManager.Instance.TutorialSeq <= 14)
				{
					tt.index = 17;
				}
				else if (weaponType == WeaponType.gun_m4 && SettingManager.Instance.TutorialSeq <= 14)
				{
					tt.index = 18;
				}
			}
