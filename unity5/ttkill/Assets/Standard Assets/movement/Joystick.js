...
private var myTouchZone : Rect;

private var alphaOff:float = 0;

var screenRate : float  = 1;
var rate : float = 1.0f;

function AdjustPosition(guix : float, guiy : float)
{
	gui.pixelInset.x = guix;
	gui.pixelInset.y = guiy;
	...
	else
	{
		...
		myTouchZone = defaultRect;
		myTouchZone.width = defaultRect.width/2;
		myTouchZone.height = defaultRect.height/2;
		myTouchZone.x += (guiTouchOffset.x - defaultRect.width/4);
		myTouchZone.y += (guiTouchOffset.y - defaultRect.height/4);
	}
}
...
//	gui.pixelInset.width *= screenRate;
//	gui.pixelInset.height *= screenRate;
	defaultRect = gui.pixelInset;
//	Debug.Log("?????????????? " + defaultRect.width);
	
	gui.color.a = 1;//alphaOff;
	...
	else
	{
		...
		///////
		myTouchZone = defaultRect;
		myTouchZone.x += guiTouchOffset.x;
		myTouchZone.y += guiTouchOffset.y;
		myTouchZone.width = defaultRect.width/10;
		myTouchZone.height = defaultRect.height/10;
	}
...
{	
		...
		else
		{
			for(var i : int = 0;i < count; i++)
			{
				...
				else if ( myTouchZone.Contains( touch.position ) )
				{
					shouldLatchFinger = true;
					gui.color.a = .5;
				}		
