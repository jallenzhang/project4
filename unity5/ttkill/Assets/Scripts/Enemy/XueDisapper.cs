...
public class XueDisapper : MonoBehaviour {
	...
	IEnumerator ChangeCutoff(float from, float to, float duration)
	{
		...
		while(tmp < 1.0f)
		{
			...
			GetComponent<Renderer>().material.SetFloat("_Cutoff", Mathf.Lerp(from, to, tmp));
			yield return null;
		}
	...
	IEnumerator ChangeAlpha(float from, float to, float duration)
	{
		...
		{
			tmp += rate * Time.deltaTime;
			GetComponent<Renderer>().material.SetColor("_Color", Color.Lerp(new Color(1, 1, 1, from), new Color(1, 1, 1, to), tmp));
			yield return null;
		}
