...
public class EnemyController : MonsterLove.StateMachine.StateMachineBehaviour {
	[HideInInspector]
...
//		Debug.Log("distance is:  " + distance + " AttackRange " + AttackRange);
	...
	void ExploreGolds()
	{
		...
		for (int i = 0; i < goldNum; i++)
		{
			...
			gold.GetComponent<Rigidbody>().AddForce(dir * power, ForceMode.Impulse);
		}
	}
...
//		transform.GetComponent<Animation>().Stop();
		if (GetComponent<Animation>()["idle"] != null)
			GetComponent<Animation>()["idle"].speed = 0;
		else
			GetComponent<Animation>()["walk"].speed = 0;
