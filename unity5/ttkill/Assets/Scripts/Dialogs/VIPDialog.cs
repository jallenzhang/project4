...
public class VIPDialog : DialogBase {
	...
	void Start()
	{
		GoldAddtionalObj.GetComponent<Collider>().enabled = (SettingManager.Instance.GoldAddtional == 0);
		goldDoneObj.SetActive(SettingManager.Instance.GoldAddtional == 1);
		WeaponAddtionalObj.GetComponent<Collider>().enabled = (SettingManager.Instance.WeaponAddtional == 0);
	...
	public void OnBuyGoldAddtion()
	{
		...
		GoldAddtionalObj.GetComponent<Collider>().enabled = false;
	...
	public void OnWeaponAddtional()
	{
		...
		DialogManager.Instance.PopupFadeOutMessage("Buy this item success!", Vector3.zero, 3);
		WeaponAddtionalObj.GetComponent<Collider>().enabled = false;
